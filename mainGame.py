...
from pygame_menu import Menu, themes, widgets
pygame.init()
import requests
...
def SendReview(text):
    if len(text) > 10:
        try:
            response = requests.get(f'http://localhost:5199/pythonshootgame?review={text}')
...
def NewGame():
    ...
    while 1: # бесконечный цикл
        ...
        if not running:
            ...
            generageMenu(score, top_score)


def generageMenu(score, top_score):
    menu = Menu("Shoot Game", 450, 800, theme=themes.THEME_DARK, overflow=(False, False))
    menu.add.label('Score: ' + str(score))
    menu.add.label('Top Score: ' + str(top_score))
    menu.add.button('Начать новую игру', NewGame)
    menu.add.label('Написать отзыв:')
    menu.add.text_input(
        title='Отзыв:', default='', maxwidth_dynamically_update=False, maxwidth=15,   onreturn=SendReview,
